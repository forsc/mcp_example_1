...
import win32con
import time
from google_serv import GmailService
...
async def draw_rectangle() -> dict:
    ...
    try:
        ...
        time.sleep(0.2)
        paint_window.click_input(coords=(535,90 ))
        time.sleep(0.2)
        # Get the canvas area
...
async def add_text_in_paint(text: str) -> dict:
    ...
    try:
        ...
        canvas.click_input(coords=(910, 490))
        time.sleep(0.5)
        ...
        canvas.click_input(coords=(1110, 690))
...
async def open_paint() -> dict:
    ...
    try:
        ...
        # Get the corodnates of the paint window
...
async def send_email(recipient_id: str, subject: str, message: str) -> dict:
    ...
    try:
        ...
        gm = GmailService(creds_file_path='creds.json', token_path='token.json')
        x = await gm.send_email(recipient_id=recipient_id, subject=subject, message=message)
        if x['status'] == 'success':
            return {
                "content": [
                    TextContent(
                        type="text",
                    text=f"Email sent to {recipient_id} with subject {subject} and message {message}"
                )
            ]
        }
        else:
            return {
                "content": [
                    TextContent(
                        type="text",
                        text=f"Error sending email: {x['error_message']}"
                    )
                ]
            }
    except Exception as e:
